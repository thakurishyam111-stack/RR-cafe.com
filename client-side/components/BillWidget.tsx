...
export default function BillWidget({ isOpen, onClose }) {
  ...
  const handleBulkPayment = async (method) => {
    ...
    if (appUrl) {
      ...
      alert(`Redirecting to ${method}. Processing payment for ${unpaidBills.length} bill(s)...`);
    } else {
                ...
                return (
                  <div key={order._id} className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700/60 shadow-md">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="text-xs font-bold text-slate-200">Table: {order.number}</h3>
                        <p className="text-amber-500/90 text-[10px] font-mono mt-0.5">Bill No: {order.billNo}</p>
                      </div>
        ...
        {unpaidBills.length > 0 && (
          <div className="p-4 bg-slate-950 border-t border-slate-800 rounded-t-2xl shadow-inner">
            <div className="flex justify-between items-center mb-3.5 px-1">
              <div>
                <p className="text-[10px] text-slate-400 uppercase tracking-wider font-semibold">Total Unpaid ({unpaidBills.length} Bills)</p>
                <h3 className="text-sm font-bold text-slate-200">Grand Total Amount</h3>
              </div>
              <span className="text-lg font-black text-green-400">Rs. {grandTotal}</span>
            </div>
