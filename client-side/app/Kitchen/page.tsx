...
const ItemLevelKitchenKDS = () => {
    ...
    try {
      ...
      const normalizedOrders = fetchedOrders.map(order => ({
        ...
        items: order.items.map(item => ({
          ...
          estimatedTime: item.estimatedTime !== undefined ? item.estimatedTime : 15
        }))
      }));

      if (normalizedOrders.length > previousOrdersCount.current && previousOrdersCount.current !== 0) {
        playAlertSound();
      }
  ...
  const updateItemStatus = async (orderId: string, itemId: string, newStatus: any) => {
    try {
      ...
      await axios.put(`http://localhost:8080/api/orders/${orderId}/items/${itemId}`, {
        status: newStatus,
      });
  ...
  const updateItemPrepTime = async (orderId: string, itemId: string, newTime: number) => {
    try {
      setOrders(prevOrders => 
        prevOrders.map(order => {
          if (order._id !== orderId) return order;
      ...
      console.error("Failed to update dynamic prep time minutes on selected index", err);
      getOrders();
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-300 text-slate-100 flex flex-col font-sans">
      ...
      <header className="bg-slate-900 border-b border-slate-800 sticky top-0 z-50 px-6 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div>
          <h1 className="text-xl font-black tracking-wide text-white">👨‍🍳 KITCHEN DISPLAY CONTROLLER</h1>
          <p className="text-xs text-indigo-400 mt-1">Status and prep-timers isolated per-item dashboard</p>
        </div>
        <button 
          onClick={getOrders} 
      ...
      <main className="flex-1 p-6">
        {loading && orders.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32">
            ...
            <p className="text-xs text-slate-400">Loading subdocument index parameters...</p>
          </div>
        ) : error ? (
          ...
          <div className="text-center py-24 border border-slate-800 rounded-2xl bg-slate-900/20 max-w-lg mx-auto">
            ...
            <h3 className="text-sm font-bold text-slate-300">Kitchen Queue Clean</h3>
            <p className="text-xs text-slate-500">Wait for new incoming orders...</p>
          </div>
        ) : (
                  ...
                  <div className="flex items-center gap-3">
                    {order.kitchenNote && (
                      <div className="bg-rose-500/10 border border-rose-500/20 px-3 py-1.5 rounded-lg text-rose-300 text-xs italic">
                        ⚠️ <span className="font-semibold text-rose-400 not-italic">Chef Note:</span> {order.kitchenNote}
                      </div>
                    )}
                    ...
                    {order.items.map((item) => (
                      <div 
                        ...
                        className={`bg-slate-950 p-4 rounded-xl border flex flex-col justify-between gap-4 transition-all ${
                          item.status === "Ready" ? "border-emerald-500/20 bg-emerald-500/[0.01]" : "border-slate-800/80"
                        }`}
                      >
                        ...
                        <div className="flex justify-between items-start gap-2">
                          <div>
                            ...
                            <h4 className="text-sm font-bold text-slate-100 mt-1.5">{item.title}</h4>
                          </div>
                        ...
                        <div className="bg-slate-900/40 p-3 rounded-lg border border-slate-800 flex justify-between items-center gap-2">
                          <div className="flex flex-col gap-1">
                            <span className="text-[9px] text-slate-500 uppercase font-semibold">Prep Timer</span>
                            <div className="flex items-center gap-1 bg-slate-950 p-1 rounded border border-slate-800">
                              <button
                                onClick={() => updateItemPrepTime(order._id, item._id, Math.max(2, item.estimatedTime - 2))}
                              ...
                              <button
                                onClick={() => updateItemPrepTime(order._id, item._id, item.estimatedTime + 2)}
                                className="h-5 w-5 bg-slate-800 hover:bg-slate-700 text-slate-200 text-[10px] font-bold rounded"
                              >
                                +
                              </button>
                          ...
                          <div className="flex flex-col items-end gap-1">
                            ...
                            <span className={`text-[10px] font-bold px-2 py-0.5 rounded border uppercase tracking-wider ${getItemStatusBadge(item.status)}`}>
                              {item.status}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="pt-2">
                          <label className="block text-[10px] font-bold text-slate-500 mb-1.5 uppercase">Update Stage</label>
                          <select
                            value={item.status}
                            onChange={(e) => updateItemStatus(order._id, item._id, e.target.value as any)}
                          ...
                          >
                            ...
                            <option value="Served">🍽️ Served (Delivered)</option>
                          </select>
                        </div>
                      </div>
                    ))}
